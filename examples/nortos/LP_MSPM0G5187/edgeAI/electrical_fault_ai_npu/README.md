...
This example demonstrates electrical fault detection and classification using an AI model on the MSPM0G5187 microcontroller with hardware-accelerated inference. The system classifies simulated electrical power system data for transmission line fault detection into one of six categories. The classification result is indicated by toggling onboard LEDs with different colors.

The dataset contains simulated electrical power system data sampled at 1000 Hz with six input features: three line currents (Ia, Ib, Ic) and three line voltages (Va, Vb, Vc). The data was generated by modeling a power system in MATLAB consisting of 4 generators and transformers, with faults simulated at the midpoint of the transmission line under both normal and various fault conditions.
...
The class to LED color mapping is shown below:
   - **Red**: Class 0 (No Fault)
   - **Green**: Class 1 (Line-to-Line B-C Fault)
   - **Blue**: Class 2 (Three-Phase ABC Fault)
   - **Yellow** (Red+Green): Class 3 (Single Line-to-Ground A Fault)
   - **Cyan** (Blue+Green): Class 4 (Double Line-to-Ground A-B Fault)
   - **Magenta** (Red+Blue): Class 5 (Three-Phase-to-Ground ABCG Fault)

## AI Model Information

| Property             | Value                                                       |
| -------------------- | ----------------------------------------------------------- |
| Model Architecture   | 3-layer CNN + 3 FC layers                                   |
| Number of Parameters | ~39,702                                                     |
| Input Shape          | Time-series (Ia, Ib, Ic, Va, Vb, Vc) Tensor[(1, 6, 128, 1)] |
| Output Classes       | 6                                                           |
| Quantization         | INT8                                                        |

A 3-layer quantized convolutional network with 1 pooling layer and progressive channel expansion (6→16→40→68), followed by 3 fully connected layers.
...
2. **Operation**
   ...
   - The pre-loaded test vector containing electrical signal data is fed to the AI model.
   ...
   - The LED corresponding to the detected class is turned on:
     - **Red**: No Fault (Normal operation)
     - **Green**: Line-to-Line (B-C) Fault
     - **Blue**: Three-Phase (ABC) Fault
     - **Yellow**: Single Line-to-Ground (A) Fault
     - **Cyan**: Double Line-to-Ground (A-B) Fault
     - **Magenta**: Three-Phase-to-Ground (ABCG) Fault

3. **Running the Example**
   - Compile, load, and run the application on your MSPM0 device.
   - Observe the LED indication for the classification result.
   - To test different input data, modify the test vector in `electrical_fault_testvector.h`.

## Software Details

- **AI Inference**: Runs a pre-trained CNN model using TVM runtime with NPU acceleration.
- **NPU Management**: The application handles NPU power-up, interrupt configuration, and power-down.
- **LED Indication**: Classification result is indicated by toggling onboard LEDs with 6 different color combinations.
...
- This example uses a pre-loaded test vector for demonstration. For real-world applications, integrate with current and voltage sensors.
...
- The 6-class classification covers: No Fault, LG (Line-to-Ground), LL (Line-to-Line), LLG (Line-to-Line-to-Ground), LLL (Three-Phase), and LLLG (Three-Phase-to-Ground) fault types.
...
- Electrical Fault Detection Dataset: [Kaggle](https://www.kaggle.com/datasets/esathyaprakash/electrical-fault-detection-and-classification)
